...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 3, 0) && defined (BUILD_AS_DKMS)
void kcl_amdgpu_connector_virtual_dpms(struct drm_connector *connector, int mode)
{
	return;
}
#else
int kcl_amdgpu_connector_virtual_dpms(struct drm_connector *connector, int mode)
{
	return 0;
}
#endif
