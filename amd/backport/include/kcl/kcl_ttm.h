...
extern int _kcl_ttm_bo_move_ttm(struct ttm_buffer_object *bo,
		bool evict, bool interruptible,
...
static inline int kcl_ttm_bo_move_memcpy(struct ttm_buffer_object *bo,
		bool evict, bool interruptible,
		bool no_wait_gpu,
		struct ttm_mem_reg *new_mem)
{
#if defined(BUILD_AS_DKMS)
	return _kcl_ttm_bo_move_memcpy(bo, evict, interruptible,
			no_wait_gpu, new_mem);
#else
...
static inline int kcl_ttm_bo_move_accel_cleanup(struct ttm_buffer_object *bo,
		struct fence *fence,
...
{
#if defined(BUILD_AS_DKMS)
	return _kcl_ttm_bo_wait(bo, interruptible, no_wait);
#else
	return ttm_bo_wait(bo, interruptible, no_wait);
#endif
}
